...
# Laura branch/route; update as needed
#################################################
@app.route("/IndyCar/speed/<driver>")
def speed(driver):
    results = session.query(
        indydata_2024.race_num,
        indydata_2024.race_city,
        indydata_2024.avg_speed,
        indydata_2024.status  # Ensure that status is a valid field
    ...
    for row in results:
        ...
        data.append({
            ...
            'status': row.status  # Ensure this field exists
        })

    graph_data = pd.DataFrame(data)

    # Create a bar chart for average speed and status
    fig = px.bar(graph_data, x='race_label', y='average_speed', color='status',
                 title=f"Average Speed of {driver} by Race")  # Change to bar graph
    fig.update_layout(
    ...
    # Return the rendered template with the graph
